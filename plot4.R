...
png(filename="plot4.png", width=480, height=480, pointsize=12, type="cairo", bg="transparent")
...
# Plot 1
with (df, {
    plot(Global_active_power ~ DT, ylab="Global Active Power", xlab="", main="", pch="", col=mainColor)
...
# Plot 2
with (df, {
    plot(Voltage ~ DT, ylab="Voltage", xlab="datetime", main="", pch="")
    lines(x=DT, y=Voltage, col=mainColor)
    message("plot2")
})

# Plot 3
with (df, {
    plot(Sub_metering_1 ~ DT, ylab="Energy sub metering", xlab="", main="", pch="")
    lines(x=DT, y=Sub_metering_1, col=mainColor)
    lines(x=DT, y=Sub_metering_2, col="red")
    lines(x=DT, y=Sub_metering_3, col="blue")
    legend("topright", bty="n", col = c("black", "blue", "red"), 
           legend = c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"), 
           lty=c(1,1,1), pt.cex=1.05, cex=0.96, x.intersp=0.88, y.intersp=1.05)
    message("plot3")
})
...
# Plot 4
with (df, {
    plot(Global_reactive_power ~ DT, ylab="Global_reactive_power", main="", xlab="datetime", 
         pch="", lwd=0.8)
    lines(x=DT, y=Global_reactive_power, col=mainColor)
    message("plot4")
})
