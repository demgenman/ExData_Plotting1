...
png(filename="plot3.png", width=480, height=480, pointsize=12, type="windows", bg="transparent")

with (df, {
    plot(Sub_metering_1 ~ DT, ylab="Energy sub metering", xlab="", main="", pch="")
    lines(x=DT, y=Sub_metering_1)
    lines(x=DT, y=Sub_metering_2, col="red")
    lines(x=DT, y=Sub_metering_3, col="blue")
    legend("topright", col = c("black", "blue", "red"), 
           legend = c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"), 
           pt.cex=1.05, cex=0.96, x.intersp=0.88, y.intersp=1.05, lty=c(1,1,1))
})
